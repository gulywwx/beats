...
func eventsMapping(r mb.ReporterV2, m *MetricSet, info opendistro.Info, content []byte) error {
	nodesStruct := struct {
	...
	masterNodeID, err := opendistro.GetMasterNodeID(m.HTTP, m.HTTP.GetURI())
	if err != nil {
		return err
	}
	...
	for nodeID, node := range nodesStruct.Nodes {
		...
		nodeData, err := schema.Apply(node)
		if err != nil {
		...
		mlockall, err := opendistro.IsMLockAllEnabled(m.HTTP, m.HTTP.GetURI(), nodeID)
		if err != nil {
